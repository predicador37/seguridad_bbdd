...
\textit{Structured Query Language}, o SQL, es el lenguaje utilizado \textbf{tanto para la definición como para la manipulación de datos} en los SGBD relacionales. Se trata de uno de los lenguajes de bases de datos más populares y en uso en la industria. Entre sus características se pueden destacar:
\begin{itemize}
...
\begin{frame}[fragile]
	...
	En primer lugar, será necesario descargar la base de datos sakila en el directorio \mintinline{text}{\home\vagrant}, por ejemplo,  y descomprimirla:
...
\begin{frame}[fragile]
 ...
 Liste el "top five" de géneros en ingresos brutos en orden descendente.  genres in gross revenue in descending order. 
 \begin{alertblock}{Atención}
 	Puede necesitar las tablas \textit{category, film\_category, inventory, payment y rental}
 \end{alertblock}
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item La sintaxis es: \mintinline{sql}{'username'@'hostname'}
		\item Una cuenta para un único usuario puede definirse así: \mintinline{sql}{'username'@'%'}.
...
\begin{frame}
	\frametitle{Resumen}
		\begin{itemize}
			\item SQL es un lenguaje declarativo que permite consultar y modificar la estructura de una base de datos relacional.
			\item Entre sus características más orientadas a garantizar la seguridad de la información se encuentran la posibilidad de crear vistas, otorgar privilegios de usuario y asignar roles.
			\item Una cuestión esencial para asegurar una base de datos relacional es disponer de una política de gestión de usuarios, roles y privilegios robusta y sometida a revisión periódica.
			\end{itemize}
\end{frame}
%------------------------------------------------
