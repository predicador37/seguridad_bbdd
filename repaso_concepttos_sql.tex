...
\begin{frame}[fragile]
	\frametitle{DML: DELETE, TRUNCATE}
	\setbeamerfont{block body}{size=\small}
...
\begin{frame}[fragile]
	\frametitle{Vistas}
\begin{exampleblock}{Sintaxis en MariaDB}
	\begin{verbatim}
		CREATE
		[OR REPLACE]
...
\begin{frame}[fragile]
	\frametitle{Privilegios SQL: tablas relacionadas}
	La información sobre privilegios se almacena en tablas de sistema:
	
\begin{itemize}
	\item user: cuentas de usuario, privilegios globales y otras columnas.
	\item db: privilegios a nivel de base de datos.
	\item host: en las últimas versiones de MariaDB ya no existe esta tabla.
	\item tables\_priv: privilegios a nivel de tabla.
	\item columns\_priv: privilegios a nivel de columna.
	\item procs\_priv: privilegios a nivel de funciones y procedimientos almacenados.
	\item proxies\_priv: privilegios a nivel de proxy.
